...
export default function Page() {
  ...
  useEffect(() => {
    ...
    try {
      // Persist normalized state so “bad shapes” don't stick
  ...
  const openCreateVendor = () => {
    // Draft = human input fields; final model is built on submit.
    const v = emptyVendor();
    setVendorForm({
      open: true,
      mode: "create",
      draft: {
        ...v,
        // keep one empty scenario by default
        scenarios: [emptyScenario()],
        // keep tiering defaults
  ...
  const deleteVendor = (vendorId) => {
    setState((p) => {
      ...
      // normalizeState will auto-add a vendor if list becomes empty
  ...
  return (
    <div className="container" style={{ padding: 22, maxWidth: 1200, margin: "0 auto" }}>
      ...
      {/* Context bar (for non-Vendors tabs) */}
      {showContextBar ? (
        <div style={{ marginTop: 14 }}>
          <Card style={{ padding: 12 }}>
