    ...
    <div className="container" style={{ padding: 22, maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 14 }}>
        {vendorForm.open ? (
          ...
          <PlaceholderView
            title="Tiering"
            text={`Tiering placeholder — Selected vendor: ${selectedVendor?.name?.trim() ? selectedVendor.name : "(Unnamed)"} — scenario: ${selectedScenario?.title?.trim() ? selectedScenario.title : "(Untitled)"}`}
          />
        ) : activeView === "Quantify" ? (
          <PlaceholderView
            title="Quantify"
            text={`Quantify placeholder — Selected vendor: ${selectedVendor?.name?.trim() ? selectedVendor.name : "(Unnamed)"} — scenario: ${selectedScenario?.title?.trim() ? selectedScenario.title : "(Untitled)"}`}
          />
        ) : activeView === "Treatments" ? (
          <PlaceholderView title="Treatments" text="Treatments placeholder (stable)." />
        ) : activeView === "Decisions" ? (
          <PlaceholderView title="Decisions" text="Decisions placeholder (stable)." />
        ) : activeView === "Dashboard" ? (
          <PlaceholderView title="Dashboard" text="Dashboard placeholder (stable)." />
        ) : (
