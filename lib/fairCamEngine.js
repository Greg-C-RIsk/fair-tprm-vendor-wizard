...
export function getScenarioControls(scenario) {
  ...
  if (Array.isArray(scenario?.treatments)) return scenario.treatments; // backward compat
  return [];
}
...
export function sampleOnceWithControls(qIn, controls, rnd) {
  ...
  // --- Base frequency chain (derive TEF/Susc/LEF)
  let tef = 0;
  let susc = 0;
  let lef = 0;
  ...
  // --- Apply LEC effects (sample OpEff per control, with VMC improving reliability)
  const tefReductions = [];
  const lefReductions = [];
  const lmReductions = [];
  ...
  // Recompute LEF + apply explicit LEF reductions
  let lefCut = 0;

  if (q.level === "LEF") {
    // Collapse all frequency-side reductions into LEF itself (because LEF is directly elicited)
