...
//
// IMPORTANT (units):
// - Probability of Action and Susceptibility are modeled as probabilities in [0..1] (NOT percent).
//   Example: 0.3 = 30%
...
export const exceedanceCurve = (values, points = 60) => {
  ...
  // Training-friendly: approximate exceedance based on quantile position
...
function normalizeSusceptibilityMode(mode) {
  // Backward-compat:
  // - UI/model may store "Derived"
  // - Engine historically used "FromCapacityVsResistance"
  if (mode === "Derived") return "FromCapacityVsResistance";
  if (mode === "FromCapacityVsResistance") return "FromCapacityVsResistance";
  return "Direct";
}
...
export const ensureQuant = (scenarioQuant) => {
  ...
  return {
    ...
    // susceptibility mode: "Direct" | "FromCapacityVsResistance" (alias: "Derived")
    susceptibilityMode: normalizeSusceptibilityMode(q.susceptibilityMode),
    ...
    // Probabilities are 0..1 (NOT percent)
    probabilityOfAction: tri(q.probabilityOfAction), // 0..1
    susceptibility: tri(q.susceptibility), // 0..1

    // Capability vs resistance (scores)
...
export const validateQuantForRun = (quant) => {
  ...
  if (q.level !== "LEF") {
    if (q.susceptibilityMode === "Direct") {
      ...
      missing.push('Susceptibility mode must be "Direct" or "FromCapacityVsResistance" (alias: "Derived")');
    }
  }

  // Range checks for probability inputs (0..1)
  const check01 = (label, t) => {
    ["min", "ml", "max"].forEach((k) => {
      const v = Number(t?.[k]);
      if (!Number.isFinite(v)) return;
      if (v < 0 || v > 1) missing.push(`${label} (${k}) must be between 0 and 1`);
    });
  };

  if (q.level === "Contact Frequency") check01("Probability of Action", q.probabilityOfAction);

  if (q.level !== "LEF" && q.susceptibilityMode === "Direct") {
    check01("Susceptibility", q.susceptibility);
  }
...
export const sampleModelOnce = (qIn, rnd = Math.random) => {
  ...
  if (q.level === "LEF") {
    ...
    const poa = clamp01(
      triangularSample(q.probabilityOfAction.min, q.probabilityOfAction.ml, q.probabilityOfAction.max, rnd)
    );
    const tef = cf * poa;
  ...
  const slef = Math.max(
    0,
    triangularSample(q.secondaryLossEventFrequency.min, q.secondaryLossEventFrequency.ml, q.secondaryLossEventFrequency.max, rnd)
  );
  const slm = Math.max(
    0,
    triangularSample(q.secondaryLossMagnitude.min, q.secondaryLossMagnitude.ml, q.secondaryLossMagnitude.max, rnd)
  );
